...
function mergeHttpPath($path1, $path2){
...
function parseInput($str, $allowCache = false){
...
function getIDAnalyzerAPIKey(){
    return getenv('IDANALYZER_KEY', true) ?: getenv('IDANALYZER_KEY');
}
function getIDAnalyzerEndpoint($requestUri){
    ...
    if(strtolower($region) === 'eu'){
        return mergeHttpPath("https://api2-eu.idanalyzer.com/",$requestUri);
    }else if(substr(strtolower($region),0,4)=="http"){
        return mergeHttpPath($region,$requestUri);
    }else{
        return mergeHttpPath("https://v2-us1.idanalyzer.com/",$requestUri);
    }
}
 ...
 * @throws Exception
 */
function callIDAnalyzerAPI($method, $path, $payload, $timeout = 60, $throwException = false, $saveFile = false){

    $ch = curl_init();
    $url = getIDAnalyzerEndpoint($path);
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json', 'X-Api-Key: '. getIDAnalyzerAPIKey()));
